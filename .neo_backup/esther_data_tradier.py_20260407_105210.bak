...
class OptionQuote(BaseModel):
    ...
    @classmethod
    def coerce_greeks(cls, v):
        """Accept OptionGreeks object or dict — Pydantic v2 safe coercion."""
        if v is None:
            return None
        if isinstance(v, OptionGreeks):
            return v  # Already correct type — pass through
        if isinstance(v, dict):
            return OptionGreeks(**v)
        return None  # Unknown type — drop greeks rather than crash
...
class TradierClient:
    ...
    async def _request(
        ...
        last_error = "unknown error"  # Neo fix: ensure last_error always defined
        ...
        for attempt in range(self.MAX_RETRIES):
            ...
            try:
                ...
                # Retry on 4xx errors too (except 429 which is handled above)
                if e.response.status_code >= 400 and attempt < self.MAX_RETRIES - 1:
                    await asyncio.sleep(delay)
                    delay *= 2
                    continue
                raise
                ...
                last_error = f"Request timeout after {self._client.timeout}s"
                ...
                # Catch any other errors and preserve details
                ...
                if hasattr(e, 'response'):
                    try:
                        response_status = getattr(e.response, 'status_code', None) or getattr(e.response, 'status', None)
                        response_headers = dict(getattr(e.response, 'headers', {}))
                        if hasattr(e.response, 'text'):
                            error_details = f"{str(e)} - Status: {response_status} - Response: {e.response.text}"
                        elif hasattr(e.response, 'content'):
                            error_details = f"{str(e)} - Status: {response_status} - Response: {e.response.content}"
                    except:
                ...
                # Store the detailed error immediately
                last_error = error_details
                
                # Special handling for order submission errors
                if "/orders" in path and method == "POST":
                    # Check for specific Tradier error conditions
                    if response_status == 503:  # Service unavailable
                        logger.warning("tradier_service_unavailable", attempt=attempt)
                        if attempt < self.MAX_RETRIES:
                            await asyncio.sleep(10.0)  # Wait longer for service recovery
                            continue
                    ...
                    if attempt >= 2:
                        break
                    # Longer delay for order retries to let broker recover
        ...
        # Include last error details in the exception
        raise RuntimeError(f"Max retries ({self.MAX_RETRIES}) exceeded for {path}. Last error: {last_error}")
